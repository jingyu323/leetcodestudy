 ...
 * 例如：a->b, a->c
 * 有空闲字母的一定可以转换
 * 没有上游的字母就视为空闲字母，因为没有上游的字母可能没在转换链中出现，
 * 也可能在无环链末端(a->b->c->d,a空闲) 也可能在环链的上游侧链末端(a->b->c->d->a,f->e->a,f空闲)，
 * 后面两种经过转换后，末端的字母就会空闲出来
 * 无环链自身可以从头到尾依次转换；有环链可以借助空闲字母完成转换
 * 单独把1这种情况提出来，是因为 abcdefghijklmnopqrstuvwxyz -> abcdefghijklmnopqrstuvwxyz
 * 这种情况所有字母的本身即为上游，没有空闲字母，所以要单独放在第一种情况来处理
 */
